...
const CATEGORY_CONFIG = {
  marketing: {
    ...
    accentColor: '#FF419D',
    gradient: 'linear-gradient(135deg, #FFB76C, #FF419D)',
  },
  creative: {
...
export default function ServicesPage() {
  ...
  const translatedServices = services.map((s, i) => ({ ...s, ...t.servicesList[i] }))

  const marketingServices = translatedServices.filter(s => s.categories?.includes('marketing') ?? s.category === 'marketing')
  const creativeServices  = translatedServices.filter(s => s.categories?.includes('creative')  ?? s.category === 'creative')
  const webServices       = translatedServices.filter(s => s.categories?.includes('web')       ?? s.category === 'web')

  const filteredSingle = activeFilter !== 'all'
              ...
              >
                {activeFilter === tab.id && (
                  <motion.span
                    ...
                    style={{
                      background: CATEGORY_CONFIG[activeFilter as keyof typeof CATEGORY_CONFIG]?.gradient
...
function CategoryHeader({ label }: { label: string; accentColor: string; accentColorRgb: string }) {
  return (
    <div className="px-1">
